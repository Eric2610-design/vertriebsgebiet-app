...
export default function MapPage() {
  ...
  const manufacturersFiltered = useMemo(() => {
    const s = filterQ.trim().toLowerCase();
    if (!s) return manufacturers;
    return manufacturers.filter((m) => (m.label ?? "").toLowerCase().includes(s) || (m.key ?? "").toLowerCase().includes(s));
  }, [manufacturers, filterQ]);

  const repsFiltered = useMemo(() => {
    const s = filterQ.trim().toLowerCase();
    if (!s) return repsList;
    return repsList.filter((r) => r.name.toLowerCase().includes(s) || r.email.toLowerCase().includes(s));
  }, [repsList, filterQ]);
...
const visibleWithGeo = useMemo(() => {
  ...
  const L = leafletRef.current as any;
  // Flyer: logo marker (circular), centered on the coordinate
  return L.divIcon({
    className: "",
    html: `<div style="width:36px;height:36px;border-radius:999px;background:#fff;box-shadow:0 2px 10px rgba(0,0,0,.25);display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,.9);">
...
const icon =
  (hasFlyer ? flyerIcon : defaultIcon) ??
  L.divIcon({
    className: "",
    html: hasFlyer
    ...
    iconSize: hasFlyer ? [36, 36] : [28, 40],
    iconAnchor: hasFlyer ? [18, 18] : [14, 40],
    ...
    <main className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          ...
          <details className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-soft">
            <summary className="cursor-pointer select-none text-sm font-semibold">Filter</summary>
            <div className="mt-3">
              <Input
                placeholder="Suchen (Hersteller/Außendienst)"
                value={filterQ}
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-3">
                ...
                <div className="mt-2 max-h-64 overflow-auto pr-1">
                  ...
                  {manufacturersFiltered.map((m) => (
                    <label key={m.key} className="flex items-center gap-3 rounded-xl px-2 py-2 text-sm hover:bg-slate-50 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={selectedManu[m.key] ?? true}
                        onChange={(e) => setSelectedManu((p) => ({ ...p, [m.key]: e.target.checked }))}
                      />
                      <BrandLabel manufacturerKey={m.key} label={m.label} />
                    </label>
                  ))}
                  </div>
                </div>
              ...
              <div className="rounded-2xl border border-slate-200 p-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => {
                        const next: Record<string, boolean> = {};
                        for (const r of repsList) next[r.email] = true;
                    ...
                    <button
                      ...
                      onClick={() => {
                        const next: Record<string, boolean> = {};
                        for (const r of repsList) next[r.email] = false;
                        setSelectedReps(next);
                      }}
                      type="button"
                    >
                ...
                <div className="mt-2 max-h-64 overflow-auto pr-1">
                  {repsList.length ? (
                    <div className="grid grid-cols-1 gap-1">
                      {repsFiltered.map((r) => (
                        <label key={r.email} className="flex items-center gap-3 rounded-xl px-2 py-2 text-sm hover:bg-slate-50 cursor-pointer">
                          <input
                            type="checkbox"
                        ...
                        <button className="text-left" onClick={() => openDealerOnMap(d.id)} type="button">
                          ...
                          <div className="mt-1 flex flex-wrap gap-1">
                            {(keys2 as string[]).slice(0, 3).map((k: string) => (
                              (() => {
                                const label = k === "flyer" ? "FLYER" : (labelByKey.get(k) ?? k);
                                const src = k === "flyer" ? "/brands/flyer.png" : getBrandIcon({ key: k, label });
                                return (
                                  <Badge key={k} tone={k === "flyer" ? "blue" : "slate"} title={label}>
                                    {src ? <img src={src} alt={label} className="h-4 w-4" /> : null}
