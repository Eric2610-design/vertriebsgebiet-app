...
export default function MapPage() {
  ...
  const runGeoDupIgnore = async (group: any) => {
    const master = geoMaster[group.key] ?? group.suggested_master_id ?? group.dealers?.[0]?.id;
    const picks = geoSelected[group.key] ?? {};
    const ignore_ids = Object.entries(picks)
      .filter(([, v]) => v)
      .map(([k]) => k)
      .filter((x) => x && x !== master);
    if (ignore_ids.length === 0) return alert("Bitte mindestens einen Händler markieren, der NICHT zum Master gehört.");
    if (!confirm(
      `Als NICHT-Duplikat speichern?\n\nMaster bleibt: ${group.dealers?.find((d: any) => d.id === master)?.name ?? master}\nNicht-Duplikate: ${ignore_ids.length}\n\nDiese Paare werden künftig nicht mehr als Dubletten vorgeschlagen.`
    )) return;
    const res = await fetch("/api/duplicates/ignore", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ dealer_id: master, ignore_ids, reason: "map_geo_not_duplicate" }),
    });
    const js = await res.json().catch(() => ({}));
                    ...
                    <div key={g.key} className="rounded-xl border border-slate-200 bg-white p-3">
                      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Button className="h-8" variant="secondary" onClick={() => runGeoDupIgnore(g)}>
                            Nicht Duplikat
                          </Button>
                        </div>
                      </div>
                            ...
                            {(g.dealers ?? []).map((d: any) => (
                              <tr key={d.id} className="border-b border-slate-100 align-top">
                                ...
                                <td className="py-2 pr-2">
                                  ...
                                  {ignored.has(d.id) ? (
                                    <div className="mt-1"><Badge tone="slate">Nicht-Duplikat</Badge></div>
                                  ) : null}
