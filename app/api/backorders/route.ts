...
function isCH(country: string | null | undefined): boolean {
...
export async function GET(req: Request) {
  ...
  // 2) Load backorder rows (MVP: no territory filter yet)
  ...
  const safeRows: BackorderRow[] = (rows ?? []) as any;

  // 3) Compute global per-article sequence (oldest first)
  const byArticle = new Map<string, BackorderRow[]>();
  ...
  for (const [article, arr] of byArticle.entries()) {
    arr.sort((a, b) => {
  ...
  // 4) Optional: frame size from latest stock snapshot
  ...
  try {
    ...
    if (stockRun?.id) {
      ...
      // supabase IN limit could be tight; chunk
      const chunkSize = 500;
    ...
    // If stock tables are not present yet, we simply omit the frame size.
    frameByArticle = {};
  }

  const result = safeRows.map((r) => {
    const country = r.dealer_country ?? null;
    return {
      ...r,
