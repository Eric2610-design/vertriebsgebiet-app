...
export async function GET(req: Request) {
  ...
  // We allow up to 10k rows per request, but we still page internally.
  ...
  let safeRows: Row[] = [];
  try {
    safeRows = await fetchAllPaged<Row>(
      (from, to) => {
        let query = supabase
        ...
        if (q) {
          query = query.or(`article_no.ilike.%${q}%,customer_no.ilike.%${q}%,dealer_name.ilike.%${q}%`);
        }
    ...
    return NextResponse.json({ error: e?.message ?? "Failed to load backorders" }, { status: 500 });
  }
  ...
  try {
    ...
    if (stockRun?.id) {
      const uniqueArticles = Array.from(
        new Set(safeRows.map((r) => String(r.article_no)).filter(Boolean))
      );
      const chunkSize = 500;
