...
supabase.from("app_settings").select("value").eq("key", "pricing_thresholds").maybeSingle(),
  ...
  if (q) {
    ...
    // NOTE: Supabase .or does not support parameter binding; we escape % above.
    q1 = q1.or(
      [
        `sku.ilike.%${q}%`,
        `name.ilike.%${q}%`,
  ...
  const itemsRes = await q1.order("model", { ascending: true }).order("model_year", { ascending: false }).limit(limit);
  if (itemsRes.error) return bad(itemsRes.error.message, 500);
  const rows = (itemsRes.data ?? []) as StockItemRow[];
