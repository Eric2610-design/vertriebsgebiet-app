  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const role = String(mj?.role || "").toLowerCase();
        if (alive) {
          setIsAdmin(role === "admin" || role === "superadmin" || !!mj?.is_admin);
        ...
        <Card>
          <CardHeader className="flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-slate-600">Fehlende Icons hochladen (nur SuperAdmin).</div>
            </div>
