      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex items-center justify-between gap-3 rounded-xl border bg-slate-50 p-2">
              <div>
                <div className="text-sm font-medium">Excel-Template für Hersteller-Import</div>
                <div className="text-xs text-slate-600">Zum Befüllen/Weitergeben – enthält Spaltenübersicht & Beispielzeile.</div>
              </div>
              <a href="/templates/hersteller_import_template.xlsx" download>
              ...
              manus.map((m) => (
                <div key={m.key} className="flex items-center justify-between gap-3 rounded-xl border p-2">
                  <div className="flex items-center gap-2">
                    <Pictogram kind="manufacturer" k={m.key} label={m.label} dataUrl={m.icon_data_url} size={28} />
                    <div>
            ...
            {groups.length ? (
              groups.map((g) => (
                <div key={g.key} className="flex items-center justify-between gap-3 rounded-xl border p-2">
                  <div className="flex items-center gap-2">
                    <Pictogram kind="buying_group" k={g.key} label={g.label} dataUrl={g.icon_data_url} size={28} />
                    <div>
