...
const stockNow = (item: StockItem): number => Math.max(0, toNum(item.avail_now));

const buildSuggestion = (item: StockItem): number => {
  const rawValue = item.raw?.["Menge Produktions-vorschlag"];
  if (rawValue === undefined || rawValue === null || rawValue === "") {
    const total = toNum(item.avail_total);
    const now = toNum(item.avail_now);
    return Math.max(total - now, 0);
  }
  return Math.max(0, toNum(rawValue));
};
