...
const visibleBranchGroups = useMemo(() => {
  let arr = branchGroups;
  if (hideLinkedBranches) arr = arr.filter((g) => g.dealers.some((d) => !d.parent_dealer_id));
  const s = q.trim().toLowerCase();
  if (!s) return arr;
  return arr.filter((g) => g.dealers.some((d) => (d.name ?? "").toLowerCase().includes(s)));
}, [branchGroups, hideLinkedBranches, q]);
...
const filteredBranch = useMemo(() => {
  const s = q.trim().toLowerCase();
  let arr = visibleBranchGroups;
  if (!s) return arr;
  return arr.filter((g) => g.dealers.some((d) => (d.name ?? "").toLowerCase().includes(s)));
}, [visibleBranchGroups, q]);

  
function isAddrGroupMarked(groupKey: string) {
  const picks = mergeSelected[groupKey] ?? {};
  return Object.values(picks).some(Boolean);
}

function isBranchGroupMarked(baseName: string) {
  const picks = branchSelected[baseName] ?? {};
  return Object.values(picks).some(Boolean);
}
...
function setSel(groupKey: string, dealerId: string, checked: boolean) {
  ...
  async function linkBranchGroup(group: BranchGroup, parentId: string) {
  ...
  if (!confirm(`Alle Filial-Vorschläge verknüpfen?\n\nGruppen: ${visibleBranchGroups.length}`)) return;
  setProgress("Verknüpfe alle Filial-Gruppen…");
  for (let i = 0; i < visibleBranchGroups.length; i++) {
    const g = visibleBranchGroups[i];
    const parentId = (mergeMaster[g.base_name] ?? g.suggested_parent_id ?? "").toString();
    if (!parentId) continue;

    // auto-pick: alle ohne parent, außer parent selbst
    const autoBranches = g.dealers.filter((d) => d.id !== parentId && !d.parent_dealer_id).map((d) => d.id);
    if (autoBranches.length === 0) continue;

    // set selections so linkBranchGroup uses them
    setBranchSelected((p) => ({
      ...p,
      [g.base_name]: Object.fromEntries(autoBranches.concat([parentId]).map((id) => [id, true])),
    }));

    setProgress(`Gruppe ${i + 1}/${visibleBranchGroups.length}…`);
...
async function linkMarkedBranchGroups() {
  const marked = visibleBranchGroups.filter((g) => isBranchGroupMarked(g.base_name));
  if (marked.length === 0) return alert("Keine markierten Filial-Gruppen.");
  if (!confirm(`Markierte Filial-Gruppen verknüpfen?\n\nGruppen: ${marked.length}`)) return;

  setProgress("Verknüpfe markierte Filial-Gruppen…");
  for (let i = 0; i < marked.length; i++) {
    const g = marked[i];
    const parentId = (mergeMaster[g.base_name] ?? g.suggested_parent_id ?? "").toString();
    if (!parentId) continue;
    setProgress(`Gruppe ${i + 1}/${marked.length}…`);
    await linkBranchGroup(g, parentId);
  }
  setProgress("Fertig.");
  await load();
...
return (
    <main className="mx-auto max-w-6xl px-4 py-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            onClick={() => bulkMerge(filteredAddr.filter((g)=>isAddrGroupMarked(g.key)), "Markierte Gruppen sofort mergen")}
            disabled={filteredAddr.filter((g)=>isAddrGroupMarked(g.key)).length === 0 || loading}
          >
            Markierte mergen
          </Button>
                  ...
                  return (
                    <div key={g.key} className="rounded-2xl border border-slate-200 bg-white p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2"><Button onClick={() => runGroupMerge(g)}>Mergen</Button></div>
                      </div>
  ...
  <div className="flex flex-wrap items-center gap-2">
    ...
    <Button variant="secondary" onClick={linkAllBranchGroups} disabled={visibleBranchGroups.length===0 || loading}>Alle verknüpfen</Button>
    <Button variant="secondary" onClick={linkMarkedBranchGroups} disabled={visibleBranchGroups.filter((g)=>isBranchGroupMarked(g.base_name)).length===0 || loading}>Markierte verknüpfen</Button>
  </div>
</CardHeader>
            <CardContent className="space-y-4">
