...
type AddrGroup = { key: string; address: string; dealers: D[]; suggested_master_id: string; ignored_with_master?: string[] };

type BranchGroup = { base_name: string; dealers: D[]; suggested_parent_id: string };

type ZipGroup = { zip: string; dealers: D[]; suggested_master_id: string; ignored_with_master?: string[] };

type GeoGroup = { key: string; zip: string; min_distance_m: number; dealers: D[]; suggested_master_id: string; ignored_with_master?: string[] };
...
export default function CleanupPage() {
  ...
  const [zipMaster, setZipMaster] = useState<Record<string, string>>({}); // zip -> master_id
  const [zipSelected, setZipSelected] = useState<Record<string, Record<string, boolean>>>({}); // zip -> {dealerId: bool}
  const [zipForce, setZipForce] = useState<Record<string, boolean>>({});

  const [geoMaster, setGeoMaster] = useState<Record<string, string>>({}); // geo.key -> master_id
  const [geoSelected, setGeoSelected] = useState<Record<string, Record<string, boolean>>>({}); // geo.key -> {dealerId: bool}
  ...
  async function load() {
    ...
    try {
      ...
      setZipGroups(js.zip_clusters ?? []);
      setGeoGroups(js.geo_duplicates ?? []);
    } finally {
      setLoading(false);
    }
  }
  ...
  const filteredZip = useMemo(() => {
    const s = q.trim().toLowerCase();
    if (!s) return zipGroups;
    return zipGroups.filter((g) => g.zip.toLowerCase().includes(s) || (g.dealers ?? []).some((d:any)=>String(d.name??" ").toLowerCase().includes(s)));
  }, [zipGroups, q]);

  const filteredGeo = useMemo(() => {
    const s = q.trim().toLowerCase();
    if (!s) return geoGroups;
    return geoGroups.filter((g) => g.zip.toLowerCase().includes(s) || (g.dealers ?? []).some((d:any)=>String(d.name??" ").toLowerCase().includes(s)));
  }, [geoGroups, q]);
...
Hinweis: Dieser Merge ignoriert Adresse/Land/PLZ-Checks.`)) return;
  ...
  function setZipSel(zip: string, dealerId: string, checked: boolean) {
    setZipSelected((prev) => {
      const g = { ...(prev[zip] ?? {}) };
      g[dealerId] = checked;
      return { ...prev, [zip]: g };
  ...
  async function runZipMerge(zip: string) {
    const masterId = zipMaster[zip];
    if (!masterId) return alert("Bitte zuerst einen Master auswählen.");
    const ids = Object.entries(zipSelected[zip] ?? {})
      .filter(([, v]) => v)
      .map(([id]) => id)
      .filter((id) => id !== masterId);
    if (ids.length === 0) return alert("Bitte mindestens einen Händler zum Mergen auswählen.");

    const force = !!zipForce[zip];
    if (!confirm(`Merge durchführen?

PLZ: ${zip}
Master: ${masterId}
Mergen: ${ids.length} Händler

${force ? "Hinweis: FORCE ist aktiv (ignoriert Adresse/Land/PLZ-Checks)." : "Hinweis: Ohne Force ist Merge nur erlaubt, wenn Adresse identisch ist."}`)) return;

    const res = await fetch("/api/merge", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ master_id: masterId, merge_ids: ids, reason: "cleanup_zip_cluster", force }),
    });
    const js = await res.json();
    if (!res.ok) return alert(js?.error ?? "Merge fehlgeschlagen");
    await load();
  ...
  async function runZipIgnore(zip: string) {
    const masterId = zipMaster[zip];
    if (!masterId) return alert("Bitte zuerst einen Master auswählen.");
    const ids = Object.entries(zipSelected[zip] ?? {})
      .filter(([, v]) => v)
      .map(([id]) => id)
      .filter((id) => id !== masterId);
    if (ids.length === 0) return alert("Bitte mindestens einen Händler markieren, der NICHT zum Master gehört.");
    ...
    const res = await fetch("/api/duplicates/ignore", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ dealer_id: masterId, ignore_ids: ids, reason: "cleanup_zip_not_duplicate" }),
    });
    const js = await res.json();
    if (!res.ok) return alert(js?.error ?? "Konnte Ignore nicht speichern");
    // clear selections for this zip
    setZipSelected((prev) => ({ ...prev, [zip]: {} }));
    await load();
  }

  function setGeoSel(key: string, dealerId: string, checked: boolean) {
    setGeoSelected((prev) => {
      const g = { ...(prev[key] ?? {}) };
      g[dealerId] = checked;
      return { ...prev, [key]: g };
  ...
  async function runGeoMerge(key: string) {
    const masterId = geoMaster[key];
    if (!masterId) return alert("Bitte zuerst einen Master auswählen.");
    const ids = Object.entries(geoSelected[key] ?? {})
      .filter(([, v]) => v)
      .map(([id]) => id)
      .filter((id) => id !== masterId);
    if (ids.length === 0) return alert("Bitte mindestens einen Händler zum Mergen auswählen.");

    const force = geoForce[key] ?? true;
    if (!confirm(`Merge durchführen?
...
${force ? "Hinweis: FORCE ist aktiv (ignoriert Adresse/Land/PLZ-Checks)." : "Hinweis: Ohne Force ist Merge nur erlaubt, wenn Adresse identisch ist."}`)) return;
    ...
    const res = await fetch("/api/merge", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ master_id: masterId, merge_ids: ids, reason: "cleanup_geo_cluster", force }),
    });
    const js = await res.json();
    if (!res.ok) return alert(js?.error ?? "Merge fehlgeschlagen");
    await load();
  ...
  async function runGeoIgnore(key: string) {
    const masterId = geoMaster[key];
    if (!masterId) return alert("Bitte zuerst einen Master auswählen.");
    const ids = Object.entries(geoSelected[key] ?? {})
      .filter(([, v]) => v)
    ...
    if (ids.length === 0) return alert("Bitte mindestens einen Händler markieren, der NICHT zum Master gehört.");
    if (!confirm(
      `Als NICHT-Duplikat speichern?\n\nGeo-Gruppe: ${key}\nMaster: ${masterId}\nNicht-Duplikate: ${ids.length}\n\nDiese Paare werden künftig nicht mehr als Dubletten vorgeschlagen.`
    )) return;
    const res = await fetch("/api/duplicates/ignore", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ dealer_id: masterId, ignore_ids: ids, reason: "cleanup_geo_not_duplicate" }),
    });
    const js = await res.json();
    if (!res.ok) return alert(js?.error ?? "Konnte Ignore nicht speichern");
    setGeoSelected((prev) => ({ ...prev, [key]: {} }));
    await load();
  }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader className="text-sm font-semibold">PLZ-Kollisionen (mehrere Händler pro PLZ)</CardHeader>
            <CardContent className="space-y-4">
              <div className="text-xs text-slate-500">
                Zeigt alle Postleitzahlen, in denen mehr als ein Händler existiert (egal ob geocodiert oder nicht). Hier kannst du Dubletten direkt forcieren.
              </div>
              {filteredZip.length === 0 ? (
                <div className="text-sm text-slate-600">Keine PLZ mit mehreren Händlern gefunden.</div>
              ) : (
                filteredZip.slice(0, 120).map((g) => {
                  const zip = g.zip;
                  const masterId = zipMaster[zip] ?? g.suggested_master_id ?? (g.dealers?.[0]?.id ?? "");
                  const force = !!zipForce[zip];
                  const ignored = new Set<string>(g.ignored_with_master ?? []);
                  return (
                    <div key={zip} className="rounded-2xl border border-slate-200 bg-white p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          <div className="text-sm font-semibold">PLZ {zip}</div>
                          <div className="text-xs text-slate-500">{g.dealers.length} Händler</div>
                        </div>
                        <div className="flex flex-wrap items-center gap-2 text-xs">
                          ...
                          <select
                            ...
                            value={masterId}
                            onChange={(e)=>setZipMaster((s)=>({ ...s, [zip]: e.target.value }))}
                          >
                            {(g.dealers ?? []).map((d:any)=> (
                              <option key={d.id} value={d.id}>{d.name}</option>
                            ))}
                          ...
                          <label className="ml-2 flex items-center gap-2">
                            <input type="checkbox" checked={force} onChange={(e)=>setZipForce((s)=>({ ...s, [zip]: e.target.checked }))} />
                            <span><b>Force</b></span>
                          </label>
                          <Button variant="secondary" onClick={()=>runZipMerge(zip)}>Merge</Button>
                          <Button variant="secondary" onClick={()=>runZipIgnore(zip)}>Nicht Duplikat</Button>
                        </div>
                      </div>
                        ...
                        {(g.dealers ?? []).map((d:any)=> (
                          <label key={d.id} className="flex items-start gap-3 rounded-lg border border-slate-100 px-3 py-2 hover:bg-slate-50">
                            <input
                              type="checkbox"
                              className="mt-1"
                              checked={!!(zipSelected[zip]?.[d.id])}
                              onChange={(e)=>setZipSel(zip, d.id, e.target.checked)}
                              disabled={d.id === masterId || ignored.has(d.id)}
                            />
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-2">
                                <div className="text-sm font-semibold truncate">{d.name}</div>
                                <div className="shrink-0">
                                  <DealerListPictos manufacturerKeys={d.manufacturer_keys ?? []} buyingGroupKey={d.buying_group_key ?? null} size={14} maxManufacturers={3} />
                                </div>
                              </div>
                              {ignored.has(d.id) && <div className="mt-1 text-xs"><Badge tone="slate">Nicht-Duplikat</Badge></div>}
          ...
          <Card>
            <CardHeader className="text-sm font-semibold">Geo-Dubletten (sehr nah beieinander)</CardHeader>
            <CardContent className="space-y-4">
              <div className="text-xs text-slate-500">
                Gruppen, bei denen Händler geocodiert sind und die Koordinaten innerhalb eines engen Radius liegen. Das ist oft ein starker Hinweis auf identische Einträge.
              </div>
              {filteredGeo.length === 0 ? (
                <div className="text-sm text-slate-600">Keine Geo-Dubletten gefunden.</div>
              ) : (
                filteredGeo.slice(0, 160).map((g) => {
                  const key = g.key;
                  const masterId = geoMaster[key] ?? g.suggested_master_id ?? (g.dealers?.[0]?.id ?? "");
                  const force = geoForce[key] ?? true;
                  const ignored = new Set<string>(g.ignored_with_master ?? []);
                  return (
                    <div key={key} className="rounded-2xl border border-slate-200 bg-white p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          <div className="text-sm font-semibold">PLZ {g.zip} · ~{g.min_distance_m} m</div>
                          <div className="text-xs text-slate-500">{g.dealers.length} Händler</div>
                        </div>
                        <div className="flex flex-wrap items-center gap-2 text-xs">
                          <span className="text-slate-600">Master:</span>
                          <select
                            className="rounded-md border border-slate-200 bg-white px-2 py-1"
                            value={masterId}
                            onChange={(e)=>setGeoMaster((s)=>({ ...s, [key]: e.target.value }))}
                          >
                            {(g.dealers ?? []).map((d:any)=> (
                              <option key={d.id} value={d.id}>{d.name}</option>
                            ))}
                          ...
                          <label className="ml-2 flex items-center gap-2">
                            <input type="checkbox" checked={!!force} onChange={(e)=>setGeoForce((s)=>({ ...s, [key]: e.target.checked }))} />
                            <span><b>Force</b></span>
                          </label>
                          <Button variant="secondary" onClick={()=>runGeoMerge(key)}>Merge</Button>
                          <Button variant="secondary" onClick={()=>runGeoIgnore(key)}>Nicht Duplikat</Button>
                          <Button variant="secondary" onClick={()=>runGeoIgnore(key)}>Nicht Duplikat</Button>
                        </div>
                      </div>
                        ...
                        {(g.dealers ?? []).map((d:any)=> (
                          <label key={d.id} className="flex items-start gap-3 rounded-lg border border-slate-100 px-3 py-2 hover:bg-slate-50">
                            <input
                              type="checkbox"
                              className="mt-1"
                              checked={!!(geoSelected[key]?.[d.id])}
                              onChange={(e)=>setGeoSel(key, d.id, e.target.checked)}
                              disabled={d.id === masterId || ignored.has(d.id)}
                            />
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-2">
