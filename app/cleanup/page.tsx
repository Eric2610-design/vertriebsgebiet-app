...
Filialen: ${child_ids.length}`)) return;
  ...
  async function normalizeAddresses() {
    if (!confirm("Adressen-Normalisierung ausführen?\n\nDabei werden nur die norm_* Felder aktualisiert (für bessere Dubletten-Erkennung).")) return;
    setRunning(true);
    setProgress("Normalisiere Adressen…");
    try {
      const res = await fetch("/api/dealers/normalize", { method: "POST" });
      const js = await res.json();
      if (!res.ok) throw new Error(js?.error ?? "Normalisierung fehlgeschlagen");
      setProgress(`Normalisiert. Geprüft: ${js.scanned}, aktualisiert: ${js.updated}. Lade Dubletten neu…`);
      await load();
      setProgress("Fertig.");
    } catch (e: any) {
...
async function bulkMerge(groups: AddrGroup[], label: string) {
    if (groups.length === 0) return alert("Keine Gruppen ausgewählt");
    if (!confirm(`${label}?\n\nGruppen: ${groups.length}\n\nPro Gruppe wird alles zusammengeführt. Merge nur bei exakt gleicher Adresse.`)) return;
    ...
    try {
      for (let i = 0; i < groups.length; i++) {
        ...
        setProgress(`Merge ${i + 1}/${groups.length}: ${g.address}`);
        ...
        try {
          ...
          if (!res.ok) {
            failCount++;
            console.warn("Bulk merge failed", g.key, js?.error ?? js);
          } else {
            okCount++;
          }
        } catch (e) {
          failCount++;
          console.warn("Bulk merge exception", g.key, e);
        }
      }
      alert(`Fertig.\n\nOK: ${okCount}\nFehler: ${failCount}`);
      await load();
    ...
    <main className="mx-auto max-w-6xl px-4 py-6">
      ...
      {running ? (
        <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3 text-sm">
          <div className="font-semibold">Bulk-Merge läuft…</div>
          ...
          <Card>
            <CardHeader className="text-sm font-semibold">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span>Adress-Dubletten (merge-fähig)</span>
                <div className="flex flex-wrap gap-2">
                  
                  <Button
                    onClick={normalizeAddresses}
                    disabled={running}
                    variant="secondary"
                  >
                    Adressen vereinheitlichen
                  </Button>

<Button
                    onClick={() => bulkMerge(filteredAddr, "Alle Adress-Dubletten sofort mergen")}
                    disabled={running || filteredAddr.length === 0}
                    variant="secondary"
                  >
                    Alle mergen
                  </Button>
                  <Button
                    ...
                    disabled={running || filteredAddr.filter((g) => groupMarked[g.key]).length === 0}
                  >
                    Markierte mergen
                  </Button>
                </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-2">
                            <input
                              type="checkbox"
                              checked={!!groupMarked[g.key]}
                              onChange={(e) => setGroupMarked((p) => ({ ...p, [g.key]: e.target.checked }))}
                            />
                        ...
                        <Button onClick={() => runGroupMerge(g)} disabled={running}>Mergen</Button>
                      </div>
