...
type MetaErr = { ok: false; error?: any; message?: string };
...
export default function AppDashboardPage() {
  ...
  async function loadMeta() {
    ...
    try {
      ...
      let json: MetaOk | MetaErr | any = null;
      try {
      ...
      if (!res.ok) {
        // falls Login abgelaufen ist -> auf /login
        if (res.status === 401 || res.status === 403) {
          router.push("/login");
          return;
        }
        throw new Error(
          json?.error?.message || json?.message || `API Fehler (${res.status})`
        );
      }

      if (!json) {
        throw new Error(
          "Unerwartete Antwort von /api/meta (kein JSON)."
        );
      }
      ...
      // Priorität: URL ?w=...  -> localStorage -> meta.workspace_id -> first workspace
      const urlW = params.get("w") || "";
      const storedW =
        typeof window !== "undefined"
          ? window.localStorage.getItem("vg.workspaceId") || ""
          : "";
      ...
      // URL sauber halten (damit man Links teilen kann)
      if (initial) {
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
        ...
        >
          <div>
            ...
            <div style={{ opacity: 0.8 }}>
              Workspace:{" "}
              <b>{workspaceLabel || (loading ? "…" : "(kein Workspace)")}</b>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            <div>
              ...
              <div style={{ marginTop: 6, opacity: 0.7, fontSize: 12 }}>
                Tipp: Die Auswahl wird gespeichert (localStorage) und auch in der URL (?w=…).
              </div>
            </div>
          </div>
        )}
      ...
      {/* Debug (optional): wenn du /api/meta prüfen willst, kannst du das hier aktivieren */}
      {/* <pre className="card" style={{ overflowX: "auto" }}>{JSON.stringify(rawMeta, null, 2)}</pre> */}
    </div>
  );
}
