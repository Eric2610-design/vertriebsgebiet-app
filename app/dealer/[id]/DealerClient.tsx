...
export default function DealerClient({ id }: { id: string }) {
  ...
  const [flyerSummary, setFlyerSummary] = useState<{ invoice_count: number; order_count: number; has_any: boolean } | null>(null);
...
async function loadFlyerSummary() {
  try {
    const res = await fetch(`/api/dealers/${id}/flyer/summary`, { cache: "no-store" });
    const js = await res.json();
    setFlyerSummary(js);
  } catch {
    setFlyerSummary({ invoice_count: 0, order_count: 0, has_any: false });
  }
}
  ...
  async function loadAll() {
    setLoading(true);
    try {
      await Promise.all([loadDealer(), loadManufacturers(), loadFlyerSummary()]);
    } finally {
      setLoading(false);
    }
  }
  ...
  const hasFlyerData = !!flyerSummary?.has_any;
  ...
  <div className="rounded-xl border border-slate-200 p-3">
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        ...
        <Badge tone="slate" title="Rechnungen">{flyerSummary?.invoice_count ?? 0} INV</Badge>
        <Badge tone="slate" title="Offene Aufträge">{flyerSummary?.order_count ?? 0} ORD</Badge>
      </div>
    </div>

    <div className="mt-2 text-xs text-slate-500">
      Dieser Bereich erscheint nur, wenn zu diesem Händler FLYER-Rechnungen oder offene Aufträge importiert wurden.
    </div>

    <div className="mt-3 grid gap-3">
      <div className="rounded-xl border border-slate-200 p-3">
        <div className="text-xs font-semibold text-slate-700">Offene Aufträge</div>
        <div className="mt-2 text-sm text-slate-700">
          {flyerSummary?.order_count ? (
            <>Es wurden {flyerSummary.order_count} offene Auftragspositionen gefunden. (Detailansicht folgt)</>
          ) : (
      ...
      <div className="rounded-xl border border-slate-200 p-3">
        <div className="text-xs font-semibold text-slate-700">Rechnungen</div>
        <div className="mt-2 text-sm text-slate-700">
          {flyerSummary?.invoice_count ? (
            <>Es wurden {flyerSummary.invoice_count} Rechnungspositionen gefunden. (Detailansicht folgt)</>
          ) : (
