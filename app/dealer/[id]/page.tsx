...
export default function DealerPage({ params }: { params: { id: string } }) {
  ...
  async function addVisit() {
    ...
    const res = await fetch(`/api/dealers/${params.id}/visits`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  async function removeManufacturer(key: string) {
    ...
    const res = await fetch(`/api/dealers/${params.id}/manufacturers/${key}`, { method: "DELETE" });
    const js = await res.json();
  ...
  async function deleteDealer() {
    ...
    const res = await fetch(`/api/dealers/${params.id}`, { method: "DELETE" });
    const js = await res.json();
    if (!res.ok) throw new Error(js?.error ?? "Löschen fehlgeschlagen");
